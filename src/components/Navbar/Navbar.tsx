...
const Navbar = ({isTopOfPage, selectedPage, setSelectedPage}: Props) => {
    ...
    return <nav>
        ...
        {!isAboveMediumScreens && isMenuToggled && (
            <div className = "fixed right-0 bottom-0 z-40 h-full w-[250px] bg-orange-100 drop-shadow-xl">
                {/* Close Icon for Menu */}
                <div className="flex justify-end p-12">
                    <button onClick={() => setIsMenuToggled(!isMenuToggled)}>
                        <XMarkIcon className="h-6 w-6 text-gray-500 hover:text-slate-50 transition duration-500"></XMarkIcon>
                    </button>
                </div>
